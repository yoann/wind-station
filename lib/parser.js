...
/**
 * First complete GPS fix in a log, without parsing the whole file.
 *
 * Exists so the day picker can label a file from a 2 KB `Range` fetch instead
 * of downloading every log in the folder. Deliberately tolerant: the head of a
 * file opens with a header, may repeat it, and almost always ends mid-row.
 *
 * @param {string} text  decoded file contents, possibly truncated
 * @returns {{lat:number, lon:number}|null}
 */
export function parseFirstFix(text) {
  const raw = String(text);
  const lines = raw.split(/\r?\n/);
  // Unless the text ended on a line break, its last line is a fragment cut by
  // the byte range — parsing it would risk a half-written coordinate.
  if (!raw.endsWith('\n')) lines.pop();
