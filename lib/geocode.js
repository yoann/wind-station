...
// v2: names are now requested in English, so anything cached under v1 is in
// the local language and must not be reused.
const CACHE_KEY = 'wind-station.places.v2';
 ...
 * @param {number} lat
 * @param {number} lon
 * @param {object} [opts]  endpoint, zoom, language, minIntervalMs, cacheDays, and the
...
export function reverseGeocode(lat, lon, opts = {}) {
  ...
  const params = new URLSearchParams({
    ...
    zoom: String(zoom),
    addressdetails: '1',
    // Nominatim answers in the local language by default, which puts a Turkish
    // or Greek name above the chart for an English page. Ask for English and
    // let it fall back to the local name where no exonym exists.
