...
// the device samples at 17-33s, so "last hour" is never "the last N rows".
...
export function summarise(rows, nowMs = null) {
  ...
  const w60 = windowRows(withWind, end, 60);
  ...
  return {
    ...
    max60: max(w60.map((r) => r.tws)),
    mean60: mean(w60.map((r) => r.tws)),
