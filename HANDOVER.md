...
| Chart readout | Dismissed by a pointer landing outside either canvas, and by scroll | On touch nothing ever took the label down: Chart.js hides on `mouseout`, which a finger never sends, and `touchend` is not in its listened set. Cleared on both charts at once, and the bubble and the crosshair are two separate Chart.js states (`tooltip._active` vs `chart._active`) that must be reset together. A drag along a chart is exempt from the scroll rule, or a scrub would dismiss its own readout |
